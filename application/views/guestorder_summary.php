...
<head>
    ...
    <title>Swarnagowri - Guest Summary </title>
                                    ...
                                    <address class="mb-4">
                                        <table class="address-table">
                                            <tbody>
                                                <?php 
                                                    if(count($billing) >0) {
                                                        $sstate = $this->master_db->sqlExecute('select s.name as sname,c.cname,a.areaname,ob.sfname,ob.sfname,ob.saddress,ob.semail from order_bills ob left join states s on s.id = ob.sstate left join cities c on c.id = ob.scity left join area a on a.id = ob.sarea where ob.oid='.$orders[0]->oid.'');
                                                       // echo $this->db->last_query();

                                                        if(count($sstate) >0 && $sstate[0]->sname !=NULL) {
                                                            ?>
                                                        <tr>
                                                    <td><?= $sstate[0]->sfname;?></td>
                                                </tr>
                                                <tr>
                                                    <td><?= $sstate[0]->semail;?></td>
                                                </tr>
                                                <tr>
                                                    <td><?= $sstate[0]->saddress;?></td>
                                                </tr>
                                                 <tr>
