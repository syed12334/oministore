        ...
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body">
                        ...
                        ?>
                         <button class="btn btn-info" data-toggle="modal" data-target="#coupons" style="float: right;margin-left: 10px;"><i class="fas fa-upload" style="margin-right: 5px"></i> Upload Coupons</button>
                        <a class="btn btn-success" href="<?= base_url().'couponsadd';?>" style="float: right;margin-bottom: 20px;margin-right: 20px"><i class="fa fa-plus"></i> Add Coupon</a>
                        <br /><br /><br />
    ...
    var dataTable, edit_data;
            function initialiseData(){
                dataTable = $('#category_table').DataTable({  
                    ...
                    "order":[],  
                    "ajax":{  
                        url:"<?=base_url().'master/getCouponlist'?>",  
                        type:"POST",
                        data: function(d){
            ...
            initialiseData();
                ...
                function updateStatus(id,status){
                       var postdata = { id : id,status : status,"<?= $this->security->get_csrf_token_name();?>":$(".csrf_token").val() } ;
                        ...
                        $.ajax({                        
                            url: "<?=base_url().'master/setcouponStatus'?>",
                            type: "post",
                            data:  postdata ,
                            dataType : 'json',
...
$(document).ready(function() {
      $("#coluponForm").on("submit",function(e) {
    e.preventDefault();
      var formdata =new FormData(this);
      $.ajax({
          url :"<?= base_url().'excelimport/saveCoupons';?>",
          method :"post",
          dataType:"json",
          data :formdata,
          contentType: false,
...
<div class="modal fade" id="coupons">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
      
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title">Upload Coupons</h4>
        ...
        <div class="modal-body">
          ...
          <form id="coluponForm" method="post" enctype="multipart/form-data">
            <input type="hidden" class="csrf_token" name="<?= $this->security->get_csrf_token_name();?>" value="<?= $this->security->get_csrf_hash();?>">
              <div class="form-group">
                  <label>Select file <font style="color:red">* (Only .xlsx,.csv are allowed)</font></label>
                  <input type="file" name="coupons" id="coupons" class="form-control">
                  <span id="file_error" class="text-danger"></span>
              </div>
              <button type="submit" id="brandSubmit" class="btn btn-primary">Submit</button>
              <a href="<?= app_asset_url().'formats/coupons.xlsx';?>" class="btn btn-success" style="margin-left: 4px"><i class="fas fa-download" style="margin-right: 6px"></i> Download Coupons Format</a>
          </form>
        </div>
