        ...
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body">
                        ...
                        ?>
                         <button class="btn btn-info" data-toggle="modal" data-target="#categoryModal" style="float: right;margin-left: 10px;"><i class="fas fa-upload" style="margin-right: 5px"></i> Upload Category</button>
                        <a class="btn btn-success" href="<?= base_url().'categoryadd';?>" style="float: right;margin-bottom: 20px;margin-right: 20px"><i class="fa fa-plus"></i> Add Category</a>
                        <br /><br /><br />
    ...
    var dataTable, edit_data;
            function initialiseData(){
                dataTable = $('#category_table').DataTable({  
                    ...
                    "order":[],  
                    "ajax":{  
                        url:"<?=base_url().'master/getCategorylist'?>",  
                        type:"POST",
                        data: function(d){
            ...
            initialiseData();
                ...
                function updateStatus(id,status){
                       var postdata = { id : id,status : status,"<?= $this->security->get_csrf_token_name();?>":$(".csrf_token").val() } ;
                        ...
                        $.ajax({                        
                            url: "<?=base_url().'master/setcategoryStatus'?>",
                            type: "post",
                            data:  postdata ,
                            dataType : 'json',
        ...
        $(document).ready(function() {
            ...
            e.preventDefault();
              ...
              $.ajax({
                  url :"<?= base_url().'excelimport/saveCategory';?>",
                  method :"post",
                  dataType:"json",
                  data :formdata,
                  contentType: false,
        ...
        <div class="modal-body">
          ...
          <form id="categoryForm" method="post" enctype="multipart/form-data">
            <input type="hidden" class="csrf_token" name="<?= $this->security->get_csrf_token_name();?>" value="<?= $this->security->get_csrf_hash();?>">
              ...
              <a href="<?= app_asset_url().'formats/category.xlsx';?>" class="btn btn-success" style="margin-left: 4px"><i class="fas fa-download" style="margin-right: 6px"></i> Download Category Format</a>
          </form>
        </div>
      </div>
    </div>
