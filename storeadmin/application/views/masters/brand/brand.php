        ...
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body">
                        ...
                        ?>
                        <button class="btn btn-info" data-toggle="modal" data-target="#brand" style="float: right;margin-left: 10px;"><i class="fas fa-upload" style="margin-right: 5px"></i> Upload Brand</button>
                        <a class="btn btn-success" href="<?= base_url().'brandadd';?>" style="float: right;margin-bottom: 20px;margin-right: 20px"><i class="fa fa-plus"></i> Add Brand</a>
    ...
    var dataTable, edit_data;
            function initialiseData(){
                dataTable = $('#category_table').DataTable({  
                    ...
                    "order":[],  
                    "ajax":{  
                        url:"<?=base_url().'master/getbrandlist'?>",  
                        type:"POST",
                        data: function(d){
            ...
            initialiseData();
                ...
                function updateStatus(id,status){
                       var postdata = { id : id,status : status,"<?= $this->security->get_csrf_token_name();?>":$(".csrf_token").val() } ;
                        ...
                        $.ajax({                        
                            url: "<?=base_url().'master/setbrandStatus'?>",
                            type: "post",
                            data:  postdata ,
                            dataType : 'json',
...
$(document).ready(function() {
    ...
    e.preventDefault();
      ...
      $.ajax({
          url :"<?= base_url().'excelimport/saveBrand';?>",
          method :"post",
          dataType:"json",
          data :formdata,
          contentType: false,
            ...
            if(data.formerror ==false) {
              ...
              $("#brandSubmit").prop('disabled',false);
               ...
               $("#processing").html('<div class="alert alert-success alert-dismissible"><i class="fas fa-circle-check"></i> '+data.msg+'</div>');
                setTimeout(function() {window.location.href="<?= base_url().'brand';?>";}, 1000);
            }
          }
      });
  });
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h4 class="modal-title">Upload Brand</h4>
        ...
        <div class="modal-body">
          ...
          <form id="brandForm" method="post" enctype="multipart/form-data">
            <input type="hidden" class="csrf_token" name="<?= $this->security->get_csrf_token_name();?>" value="<?= $this->security->get_csrf_hash();?>">
              ...
              <a href="<?= app_asset_url().'formats/brand.xlsx';?>" class="btn btn-success" style="margin-left: 4px"><i class="fas fa-download" style="margin-right: 6px"></i> Download Brand Format</a>
          </form>
        </div>
