 ...
 <div class="container-fluid my-3">
        <div class="row">
            <div class="col-md-12">
                    ...
                    <div class="card-body">
                        <div class="card-title">Sub Subcategory</div>
                        <?php 
                            if($this->session->flashdata('message')) {
                                echo $this->session->flashdata('message');
                            }
                        ?>
                        <button class="btn btn-info" data-toggle="modal" data-target="#subsubModal" style="float: right;margin-left: 10px;"><i class="fas fa-upload" style="margin-right: 5px"></i> Upload Subsubcategory</button>
                        <a class="btn btn-success" href="<?= base_url().'subsubcategoryadd';?>" style="float: right;margin-bottom: 20px;margin-right: 20px"><i class="fa fa-plus"></i> Add Sub Subcategory</a>
                        <br /><br /><br />
    ...
    var dataTable, edit_data;
            function initialiseData(){
                dataTable = $('#category_table').DataTable({  
                    ...
                    "order":[],  
                    "ajax":{  
                        url:"<?=base_url().'master/getsubsubcategorylist'?>",  
                        type:"POST",
                        data: function(d){
            ...
            initialiseData();
                ...
                function updateStatus(id,status){
                       var postdata = { id : id,status : status,"<?= $this->security->get_csrf_token_name();?>":$(".csrf_token").val() } ;
                        ...
                        $.ajax({                        
                            url: "<?=base_url().'master/setsubsubcategoryStatus'?>",
                            type: "post",
                            data:  postdata ,
                            dataType : 'json',
            ...
            $("#subsubForm").on("submit",function(e) {
            e.preventDefault();
              var formdata =new FormData(this);
              $.ajax({
                  url :"<?= base_url().'excelimport/savesubSubcategory';?>",
                  method :"post",
                  dataType:"json",
                  data :formdata,
                  contentType: false,
                    ...
                    if(data.formerror ==false) {
                      ...
                      $("#brandSubmit").prop('disabled',false);
                       ...
                       $("#processing").html('<div class="alert alert-success alert-dismissible"><i class="fas fa-circle-check"></i> '+data.msg+'</div>');
                        setTimeout(function() {window.location.href="<?= base_url().'subsubcategory';?>";}, 1000);
                    }
                  }
              });
          });
...
<div class="modal fade" id="subsubModal">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
      
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title">Upload Sub Subcategory</h4>
        ...
        <div class="modal-body">
          ...
          <form id="subsubForm" method="post" enctype="multipart/form-data">
            <input type="hidden" class="csrf_token" name="<?= $this->security->get_csrf_token_name();?>" value="<?= $this->security->get_csrf_hash();?>">
              <div class="form-group">
                  <label>Select file <font style="color:red">* (Only .xlsx,.csv are allowed)</font></label>
                  <input type="file" name="subsubcat" id="subsubcat" class="form-control">
                  <span id="file_error" class="text-danger"></span>
              </div>
              <button type="submit" id="brandSubmit" class="btn btn-primary">Submit</button>
              <a href="<?= app_asset_url().'formats/subsubcategory.xlsx';?>" class="btn btn-success" style="margin-left: 4px"><i class="fas fa-download" style="margin-right: 6px"></i> Download Subsubcategory Format</a>
          </form>
        </div>
      </div>
    </div>
